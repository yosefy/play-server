...
KalturaUrlTokenMapper = {
	...
	mapDynamicTokens: function(request, cachedUrl, playerConfig){
		...
		for(var attributeName in queryString){
			...
			KalturaLogger.debug("Evaluated attribute: [" + attributeName + "] value: [" + attributeValue + "]");		
	...
	mapTokensVod: function(request, cuePointId, cuePointUrl, partnerId, entryId, callback, apiConnector = null) {
	...
	doMapFixedTokens: function(request, cuePointId, cuePointUrl, entry, metadata, playerConfig, callback){
		...
		KalturaLogger.debug(`Saving cue point url in cache: [' + cachedUrl + '] for ${KalturaConfig.config.cache.cuePoint}`);
		KalturaCache.set(cuePointUrlKey, cachedUrl, parseInt(KalturaConfig.config.cache.cuePoint));
	...
	*/
	evaluate : function(data, objectString, defaultTokenValue, limit) {
		...
		limit = limit || 0;
		if (limit > 4) {
			KalturaLogger.debug('recursive calls are limited to 5');
			return objectString;
		}
