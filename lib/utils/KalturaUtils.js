		...
		fs.exists(options.filePath, function(exists){
			if(exists){
				...
				KalturaCache.add(fileDownloadingKey, true, KalturaConfig.config.cache.fileDownloadTimeout, function(){										
					tempPath = os.tmpdir() + '/' + KalturaUtils.getUniqueId();	
					...
					var request = http.request(options, function(response) {
						...
						localFile.on('finish', function() {
							...
							var dest = fs.createWriteStream(options.filePath);

							source.pipe(dest);
							source.on('end', function() { KalturaLogger.log('File moved from [' + tempPath + '] to [' +  options.filePath + ']'); });
							source.on('error', function(err) { KalturaLogger.log('Error moving from [' + tempPath + '] to [' +  options.filePath + ']: ' + err); });	
					    });
