...
KalturaServer.prototype.init = function() {
	this.namespace = continuationLocalStorage.createNamespace('play-server');//Here just to make sure we create it only once
...
KalturaServer.prototype.startWebServers = function() {
    ...
    if(KalturaConfig.config.cloud.httpsPort){
    	if(!KalturaConfig.config.cloud.keyFilePath || !KalturaConfig.config.cloud.certFilePath){
		    KalturaLogger.warn('Unable to locate keyFilePath || certFilePath in the configuration file. Https listener will not be created');
    		return;
    	}
...
KalturaServer.prototype.createDirs = function(){
	let dirsToCreate = [
		KalturaConfig.config.logger.convertLogDir,
	];
...
KalturaMainProcess.prototype.start = function(){
	...
	var serverProcessesKey = KalturaCache.getKey(KalturaCache.SERVER_PROCESSES_KEY_PREFIX, [os.hostname()]);
	var This = this;
	KalturaCache.get(serverProcessesKey, function(data){
		if(data){
			This.processesToRestore = data;
			KalturaLogger.log('Processes to restore [' + JSON.stringify(This.processesToRestore) + ']');
		}
...
KalturaMainProcess.prototype.storeServerProcesses = function(processes){
	KalturaLogger.log('Processes to store [' + JSON.stringify(processes) + ']');
...
KalturaMainProcess.prototype.restoreServerProcesses = function(processes){
	KalturaLogger.log('restoring server processes');
...
KalturaMainProcess.prototype.restoreServerProcess = function(pid){
	var This = this;
	var processActionsKey = KalturaCache.getKey(KalturaCache.PROCESS_ACTIONS_KEY_PREFIX, [KalturaCache.getPid(pid)]);
...
KalturaMainProcess.prototype.restoreProcessActions = function(actions){
	KalturaLogger.log('Restore process actions [' + JSON.stringify(actions) + ']');
...
KalturaChildProcess.prototype.handleRequestWithSessionId = function(request, response)
{
	const This = this;
	...
	const sessionId = Math.floor(Math.random() * (Number.MAX_SAFE_INTEGER - 1) + 1);
...
KalturaChildProcess.prototype.startHttpServer = function() {
	...
	this.httpWebServer.on('request', function(request, response) {
		return This.handleRequestWithSessionId(request, response);
	});
...
KalturaChildProcess.prototype.handleRequest = function(request, response) {
	...
	if(!service){	
		...
		try{
			var x = require(serviceModule);
			if (x[serviceClass])
				service = new x[serviceClass]();
			else
				service = new x();
		}
		catch(err){
			KalturaLogger.error('Could not initiate manager: ' + requestInfo.service);
