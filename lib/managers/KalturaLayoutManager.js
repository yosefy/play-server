...
class KalturaLayoutManager extends kaltura.KalturaManager {
	...
	manifest(request, response, params)
	{
		...
		LayoutHelper.initEntryVersion(params.entryId,
			function(){
				This._handleSessionIdVesrion(params, LayoutHelper.getBaseVersion(), request, response);
				...
				LayoutHelper.getEntryVersion(params.entryId,
					function(version){
						if(version)
							This._handleSessionIdVesrion(params, version.id, request, response);
						else
						{
	...
	_handleSessionIdVesrion(params, versionId, request, response)
	{
		const This = this;
	...
	_createFullManifestLayout(vodData, version)
	{
		...
		for (let cueIndex = 0; cueIndex < filteredCuePoints.length; cueIndex++)
		{
			...
			const encodedFetchId = KalturaLayoutManager._generateFetchId(cuePoint.id, cuePoint.duration, cuePoint.sourceUrl, vodData.getAllFlavorIds(), fillerId);
	...
	_getLayoutAPIInformation(partnerId, entryId, uiConfId, selectedFlavorIds, headers, uriPrefixFormat, callback, errorCallback)
	{
		...
		function _parseApiResults(results)
		{
			...
			{
				promises.push(This._getFlavorPath(flavorAssetList[i].id, partnerId, headers, uriPrefixFormat));
		...
		function callAPIServer()
		{
			return This._getAPIV3Information(partnerId, entryId, uiConfId).
				then(_parseApiResults, errorCallback);
		}
	...
	static _generateFetchId(cuePointId, cuePointDuration, cuePointUrl, flavorList, fillerId)
	{
	...
	updateEntry(request, response, params)
	{
		...
		response.dir(params);
		if (!this.validateActionArguments(params, ['entry_id'], response))
			return;

		const entryKey = params.entry_id;
		CacheLock.lock(entryKey,
			function() {
				This._updateVersion(entryKey, response);
			}, function() {
				KalturaLogger.debug(`Could not lock entry ${entryKey} ,this probably means someone already locked the entry`);
				setTimeout(function() {
					const entryKey = params.entry_id;
					CacheLock.lock(entryKey,
						function() {
							...
							KalturaLogger.debug(`retried to lock entry ${entryKey} and failed`);
						}
					);
	...
	_updateVersion(entryKey, response)
	{
		const This = this;
		...
		LayoutHelper.updateEntryVersion(entryKey,
			function() {
				...
				This.okResponse(response, '', 'text/plain');
			}, function(err) {
				CacheLock.unlock(entryKey);
				KalturaLogger.error(`Could not update entry ${entryKey} version, due to : ${util.inspect(err)}`);
				This.errorResponse(response, 500, `Could not update entry ${entryKey} version`);
			}
		);
	}
