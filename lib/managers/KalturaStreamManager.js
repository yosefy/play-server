...
KalturaStreamWatcher.prototype = {
	...
	stitchManifest: function(manifestContent){
		KalturaLogger.log('lowest bitrate = ' + this.lowestBitrate);
		var This = this;
		...
		for(var segmentIndex = 0; segmentIndex <= segmentsCount; segmentIndex++){
			...
			KalturaLogger.debug('Stitch segment [' + buffer[1].sequence + '] URL [' + currentSegmentUrl + 
	...
	stitchSegment: function(segments, segmentId, offset, portion){
		...
		this.manager.callPlayServerService('segment', 'stitch', this.partnerId, segmentParams);
	},
	 ...
	 */
	handleSyncPoint: function(entryId, segment){
		...
		KalturaCache.get(this.elapsedTimeKey, function(elapsedTime) {
			if(elapsedTime){
				...
				This.elapsedTime = {
						...
						offset: 0,
						timestamp: 0 // in milliseconds since 1970
				}
			}
			
			var offsetInSegment = 60000 / 90;
			var segmentOffset = This.elapsedTime.offset + 10000;
			var segmentTimestamp = 0;
			...
			KalturaLogger.log('YossiLog:: error occured while retrieving elapsed time !!!');
		})		
	},
	...
	parseCuePoints: function(entryId, segment, buffer){
		...
		this.handleSyncPoint(entryId, segment);
	},
